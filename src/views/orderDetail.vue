<template>
  <div class="w-full rounded-md" :class="order.isOpen ? 'h-[315px] border-4 border-indigo-600' : 'h-[74px] border-2'"
    ...
    <div class="mt-3 ml-2 mr-2">
      <div class="flex justify-between">
        <div class="w-32 flex justify-between">
          ...
          <img class="cursor-pointer" src="../assets/icons/share.svg" @click.stop.prevent="gotoNhanh" />
          <img class="cursor-pointer" src="../assets/icons/order.svg" @click="$emit('editOrder', order.id)" />
        </div>
      ...
      <div class="flex justify-between">
        <p class="w-[264px] h-6 text-sm text-gray-900 leading-5 whitespace-nowrap overflow-hidden text-ellipsis">
          {{ order.isOpen ? order.customerAddress : order.customerAddress?.split(',')?.[0] }}
        </p>
        <div v-if="order.createdByName" class="w-24 h-[18px] bg-gray-800 rounded-md translate-y-[80px]"
          :class="tooltip ? 'opacity-100' : 'opacity-0'">
          <p class="text-white text-xs font-medium py-[1px] text-center ">{{ order.createdByName }}</p>
        </div>
      </div>
      ...
      <div class="flex justify-start items-center mt-[5px] w-[147px]" v-show="order.isOpen">
      ...
      <div class="flex justify-between mt-[5px]" v-show="order.isOpen">
        <div class="w-44 flex justify-start items-center">
          ...
          <p class="text-xs leading-4 font-normal ml-1.5">Ngày nhận: {{ formatDay(order.deliveryDate || moment(),
              "DD/MM/YYYY")
          }}
          </p>
        </div>
      ...
      <div class="border-dashed border border-gray-400 mt-2" v-show="order.isOpen"></div>
      <div class="flex justify-between mt-2" v-show="order.isOpen">
        ...
        <div class="flex w-1/2 justify-end">
          ...
          <img @mousemove="tooltip = true" @mouseout="tooltip = false" class="ml-[5px]"
      ...
      <div class="flex justify-between mt-[5px]" v-show="order.isOpen">
        ...
        <p class="text-sky-500 text-xs whitespace-nowrap overflow-hidden text-ellipsis w-2/3 text-right">{{ order.description }}</p>
      </div>
      <div class="flex justify-between mt-[5px]" v-show="order.isOpen">
        <div class="w-44 flex justify-start items-center">
        ...
        <p class="text-sky-500 text-xs whitespace-nowrap overflow-hidden text-ellipsis w-2/3 text-right">{{ order.privateDescription }}</p>
      </div>
      <div class="border-dashed border border-gray-400 mt-2" v-show="order.isOpen"></div>
      <div class="flex justify-between mt-[5px]" v-show="order.isOpen">
        <div class="w-44 flex justify-start items-center">
        ...
        <div class="flex justify-center items-center bg-purple-100 h-5 w-[75px] rounded-[10px]">
          <p class="text-xs text-violet-800 font-medium">{{ order.tags[0] ?? 'Thêm thẻ' }}</p>
        </div>
      </div>
      <div class="flex justify-between mt-[5px]" v-show="order.isOpen">
        <div class="w-44 flex justify-start items-center">
        ...
        <div class="flex justify-center items-center bg-purple-100 h-5 w-[75px] rounded-[10px]">
          <p class="text-xs text-violet-800 font-medium">{{ order.tags[1] ?? 'Thêm thẻ' }}</p>
        </div>
      </div>
      <div class="text-sm text-yellow-500 font-medium mt-[3px]" v-show="order.isOpen">
        {{ formatCurrency(totalFee) }}
      </div>
    </div>
  </div>
