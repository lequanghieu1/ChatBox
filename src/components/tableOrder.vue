<template>
  <div class="w-full h-full shadow-md mt-2 border border-gray-200 rounded-md">
    <div
      class="w-full h-4 bg-gray-200 font-semibold flex justify-around items-center"
    >
    ...
    <div class="flex justify-between mx-2 my-1">
      ...
      />
      <div
        class="flex justify-center items-center bg-purple-100 h-5 w-[70px] rounded-[10px]"
      >
        <p
          class="text-xs text-violet-800 font-medium cursor-pointer"
          @click="filterListProduct"
        >
          Tìm kiếm
        </p>
      </div>
    ...
    <div
      class="w-56 h-full flex items-center flex-col justify-center rounded-md shadow-lg bg-white cursor-pointer"
      v-if="isHidden"
    >
      <div
        class="flex items-center justify-between w-11/12"
        v-for="item in search ? listProductFilter : list"
        :key="item.idNhanh"
      >
        <div
          class="w-2/3 flex items-center justify-start mt-2"
...
const handleChoose = (item) => {
  const tempIndex = listProducts.value.findIndex(
    (e) => e.productName === item.name
  );
  if (!item.remain) {
...
const filterListProduct = async () => {
  ...
  listProductFilter.value =
    convertObject(res.data?.data?.data?.products || {}) || [];
  isHidden.value = true;
};
</script>
